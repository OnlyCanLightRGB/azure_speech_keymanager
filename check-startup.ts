...
criticalFiles.forEach((file: string) => {
  const exists: boolean = fs.existsSync(path.join(__dirname, file));
...
const envExists: boolean = fs.existsSync(envFile);
...
// 检查依赖
console.log('\n📦 检查依赖:');
const packageJson: string = path.join(__dirname, 'package.json');
const frontendPackageJson: string = path.join(__dirname, 'frontend/package.json');
...
// 检查数据库初始化文件
console.log('\n🗄️  检查数据库:');
const dbInitFile: string = path.join(__dirname, 'database/init.sql');
...
if (fs.existsSync(examplesDir)) {
  const exampleFiles: string[] = fs.readdirSync(examplesDir);
